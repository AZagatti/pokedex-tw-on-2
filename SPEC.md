...
Git remote (SSH): `git@github.com:AZagatti/pokedex-tw-on-2.git` (already created, empty). `git init`, set remote `origin`, commit in logical chunks, push to `main`. `paths.base` must be `/pokedex-tw-on-2`. The live URL will be `https://azagatti.github.io/pokedex-tw-on-2/`.
